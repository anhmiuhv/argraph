...
class GraphInfo {
    ...
    scaler(value, axis = Axis.x) {
        switch (axis) {
            case Axis.x:
                return d3_scale_1.scaleLinear().domain([this.lowx, this.highx]).range([0, 1])(value);
            case Axis.y:
                return d3_scale_1.scaleLinear().domain([this.lowy, this.highy]).range([0, 1])(value);
            case Axis.z:
                return d3_scale_1.scaleLinear().domain([this.lowz, this.highz]).range([0, 1])(value);
            default:
                return d3_scale_1.scaleLinear().domain([this.lowx, this.highx]).range([0, 1])(value);
        }
    }
    ...
    constructor(surface) {
        ...
        for (let x = 0; x < surface.length; x++) {
            for (let y = 0; y < surface[0].length; y++) {
                data.push(new three_1.Vector3(y, surface[x][y], x));
            }
        }
        super(data);
